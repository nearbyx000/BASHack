...
def main(args):
    ...
    # Основной цикл получения изображений глубины
    while is_loop:  
        # Получение кадра с камеры глубины (depth camera)
        # camera_id: 0 - передняя камера, 1 - нижняя камера, 2 - задняя камера
        # type=CaptureType.depth - указывает на получение карты глубины
        result = client.get_camera_capture(camera_id = args.camera_num, type = CaptureType.depth)
        
        # Проверка успешности получения кадра
        if  result is not None:
            if len(result) != 0:
                # Отображение карты глубины в окне OpenCV
